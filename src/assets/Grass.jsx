...
import { PlaneGeometry, TextureLoader, Vector3, Vector4 } from "three";
...
export default function Grass({ options = { bW: 0.12, bH: 1, joints: 5 }, width = 100, instances = 50000, ...props }) {
  ...
  const [texture, alphaMap] = useLoader(TextureLoader, ["./blade_diffuse.jpg", "./blade_alpha.jpg"]);
...
function getYPosition(x, z) {
  var y = 2 * simplex.noise(x / 50, z / 50);
  y += 4 * simplex.noise(x / 100, z / 100);
  y += 0.2 * simplex.noise(x / 10, z / 10);
  return y;
}
